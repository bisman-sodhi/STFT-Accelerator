...
def top(input_A):
    ...
    # Stage 4 
    x40, x41 = butterfly(x30, cycle, buffer_40_real, buffer_40_imag, s4, pyrtl.Const(0), n)
    x42, x43 = butterfly(x31, cycle, buffer_41_real, buffer_41_imag, s4, pyrtl.Const(1), n)
    x44, x45 = butterfly(x32, cycle, buffer_42_real, buffer_42_imag, s4, pyrtl.Const(2), n)
    x46, x47 = butterfly(x33, cycle, buffer_43_real, buffer_43_imag, s4, pyrtl.Const(3), n)
    x48, x49 = butterfly(x34, cycle, buffer_44_real, buffer_44_imag, s4, pyrtl.Const(4), n)
    x50, x51 = butterfly(x35, cycle, buffer_45_real, buffer_45_imag, s4, pyrtl.Const(5), n)
    x52, x53 = butterfly(x36, cycle, buffer_46_real, buffer_46_imag, s4, pyrtl.Const(6), n)
    x54, x55 = butterfly(x37, cycle, buffer_47_real, buffer_47_imag, s4, pyrtl.Const(7), n)
...
def run_synth():
    ...
    print(f"Max timing delay: {t.max_length()} ps")
    ...
    pyrtl.synthesize()
    pre_total = 0
    for gate in pyrtl.working_block():
        if gate.op in ('s', 'c'):
            pass
        pre_total += 1
    print("Gate pre_total: " + str(pre_total))
